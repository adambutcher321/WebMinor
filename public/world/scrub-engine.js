...
function mountScrollWorld(container, config) {
  ...
  if (config.brand) {
    ...
    if (config.brand.logo) {
      const img = el('img', 'sw-brand__mark sw-brand__mark--img');
      img.src = config.brand.logo; img.alt = '';
      brand.appendChild(img);
    } else {
      brand.appendChild(el('span', 'sw-brand__mark'));
    }
    const nm = el('span', 'sw-brand__name'); nm.textContent = config.brand.name || '';
    if (config.brand.nameAccent) {
      const acc = el('span', 'sw-brand__name-accent'); acc.textContent = config.brand.nameAccent;
      nm.appendChild(acc);
    }
...
function injectCSS() {
  ...
  .sw-brand__mark--img{width:52px;height:52px;border-radius:0;background:none;box-shadow:none;object-fit:fill;}
  .sw-brand__name-accent{color:var(--sw-accent);}
